...
NOT_ON_THE_HOUR = datetime.strptime("2026/07/02 09:05", "%Y/%m/%d %H:%M")
...
def test_예약은_정시에만_가능하다_정시가_아닌경우_예약불가(booking_scheduler):
    not_on_the_hour = NOT_ON_THE_HOUR
    customer = CUSTOMER
...
def test_시간대별_인원제한이_있다_같은_시간대가_다르면_Capacity_차있어도_스케쥴_추가_성공(booking_scheduler):
    ...
    different_hout = ON_THE_HOUR + timedelta(hours=1)
    new_schedule = Schedule(different_hout, UNDER_CAPACITY, CUSTOMER)
    booking_scheduler.add_schedule(new_schedule)
...
def test_현재날짜가_일요일인_경우_예약불가_예외처리():
    booking_scheduler = TestableBookingScheduler(CAPACITY_PER_HOUR, "2026/07/05 09:00")
    schedule = Schedule(ON_THE_HOUR, UNDER_CAPACITY, CUSTOMER)
...
def test_현재날짜가_일요일이_아닌경우_예약가능():
    booking_scheduler = TestableBookingScheduler(CAPACITY_PER_HOUR, "2024/06/03 09:00")
    schedule = Schedule(ON_THE_HOUR, UNDER_CAPACITY, CUSTOMER)
